...
namespace database
{
    ...
    void Cache::put([[maybe_unused]] const std::string& email, [[maybe_unused]] const std::string &val)
    {
    ...
    bool Cache::get([[maybe_unused]] const std::string& email, [[maybe_unused]] std::string &val)
    {
        std::lock_guard<std::mutex> lck(_mtx);
        rediscpp::value response = rediscpp::execute(*_stream, "get", email);
